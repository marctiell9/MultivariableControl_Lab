function output = pplane10b(action,input1,input2,input3)

% pplane10b  is an interactive tool for studying planar autonomous systems of
%	differential equations.  When pplane10b is executed, a pplane10b Setup
...
%
%	When the Proceed button is pressed on the Setup window, the pplane10b
...
%	the pplane10b Display window, the solution to the system with that
...
if strcmp(action,'initialize')
   ...
   % First we make sure that there is no other copy of pplane10b
   % running, since this causes problems.	
   pph = findobj('tag','pplane10b');
   if ~isempty(pph);
      qstring = {'There are some pplane10b figures open although they may be invisible.  ';...
            'What do you want to do?'};
      tstring = 'Only one copy of pplane10b can be open at one time.';
      b1str = 'Restart pplane10b.';
   ...
   % Next we look for old files created by pplane10b.
   ...
   for k = 1:length(oldfiles)
      ...
      ll = fgetl(fid);
      fclose(fid);
      if strcmp(ll,'%% Created by pplane10b')
         delete(fn)
      else
	l= length(ofiles);
	ofiles{l+1} = fn;
  ...
  ud.color = color;
  ud.level = ' ';
  ppset = figure('name','pplane10b Setup',...
		 'tag','pplane10b','visible','off',...
		 'user',ud);
  
  pplane10b('figdefault',ppset);
  ...
  butt(2) = uicontrol('style','push',...
		      'pos',rwind,...
		      'string','Revert',...
		      'call','pplane10b(''revert'')',...
		      'visible','off');
  
  butt(3) = uicontrol('style','push',...
		      'pos',pwind,...
		      'string','Proceed',...
		      'call','pplane10b(''proceed'')',...
		      'visible','off');
  ...
  callnfpts = [
      'ppset = findobj(''name'',''pplane10b Setup'');',...
      'ud = get(ppset,''user'');'...
      'me = ud.h.npts;',...
  ...
  meload = uimenu(mefile,'label','Load a system ...',...
		  'MenuSelectedFcn','pplane10b(''loadsyst'',''system'');',...
		  'pos',1);
  mesave = uimenu(mefile,'label','Save the current system ...',...
		  'MenuSelectedFcn','pplane10b(''savesyst'',''system'');',...
		  'pos',2);
  meloadg = uimenu(mefile,'label','Load a gallery ...',...
		   'MenuSelectedFcn','pplane10b(''loadsyst'',''gallery'');',...
		   'separator','on','pos',3);
  mesaveg = uimenu(mefile,'label','Save a gallery ...',...
		   'MenuSelectedFcn','pplane10b(''savesyst'',''gallery'');',...
		   'tag','savegal','pos',4);
  ...
  melddg = uimenu(mefile,'label','Load the default gallery',...
		  'MenuSelectedFcn','pplane10b(''loadsyst'',''default'');',...
		  'enable','on',...
  ...
  meproceed = uimenu(mefile,...
		     'label','Proceed',...
		     'MenuSelectedFcn','pplane10b(''proceed'')',...
		     'separator','off',...
		     'accelerator','G','pos',8);
  ...
  merestart = uimenu(mefile,'label',...
		     'Restart pplane10b',...
		     'MenuSelectedFcn','pplane10b(''restart'')',...
		     'separator','on','pos',12);
  
  mequit = uimenu(mefile,...
		  'label','Quit pplane10b',...
		  'MenuSelectedFcn','pplane10b(''quit'')',...
  ...
  % Gallery menu.
   ...
   meadd = uimenu(sysmenu,'label','Add current system to the gallery',...
      'MenuSelectedFcn','pplane10b(''addgall'');','tag','add system');
   sep = 'on';
   for kk = 1:length(system)
      kkk = num2str(kk);
      if kk == 2, sep = 'off';end
      sysmen(kk) = uimenu(sysmenu,'label',system(kk).name,...
         'MenuSelectedFcn',['pplane10b(''system'',',kkk,')'],...
...
elseif strcmp(action,'loadsyst')  % This loads either a system or a gallery.
   ...
   for j = 1:nnn
      ...
      newsyst.name = sname;
      ignore = pplane10b('addgall',newsyst);
      if ignore == -1;
...
elseif strcmp(action,'proceed')
   ...
   sud.o = sud.c;
   set(ppset,'user',sud);
   he = findobj('name','pplane10b Equilibrium point data');
   hl = findobj('name','pplane10b Linearization');
   close([he;hl]);
   % set([he;hl],'vis','off');
   ...
   if (sud.flag == 1)
     ...
     NumbFPts = sud.c.npts;
     ppdisp = findobj('name','pplane10b Display');
     dud = get(ppdisp,'user');
...
%       set([arr.hx;arr.hy;arr.barrows],'vis','on');
	 ...
	 if isempty(pv) 
	   ...
	   if isempty(str2num(pv)) % This is an expression.
	     ...
	     if (err | isempty(pval))
	       ...
	       fprintf('\a')
	       errordlg(errmsg,'pplane10b error','on');
	       warning(SS)
	       return
	     end
	   end
	   xxderivstr = pplane10b('paraeval',pn,pv,xderivstr);
	   yyderivstr = pplane10b('paraeval',pn,pv,yderivstr);
     ...
     % Find pplane10b Display if it exists.	
     % If pplane10b Display exists, update it.  If it does not build it.
	 ...
	 if ~isempty(menull)
       ...
       ppdisp = figure('name','pplane10b Display',...
		       ...
		       'interrupt','on',...
		       'visible','off',...
		       'tag','pplane10b');
       pplane10b('figdefault',ppdisp);
       dud = get(ppdisp,'user');
       dud.syst = sud.c;
	...
	otherwise
	 ...
	 set(gcf,'defaultuicontrolfontsize',fs*0.9);
       ...
       dbutt(2) = uicontrol('style','push',...
			    ...
			    'string','Quit',...
			    'call','pplane10b(''quit'')','visible','off');
       
       dbutt(3) = uicontrol('style','push',...
			    'pos',[buttl,ppaxb+buttsep,buttw,butth],...
			    'string','Print',...
			    'call','pplane10b(''print'')','visible','off');	
       ...
       merestart = uimenu(fmenu,'label',...
			  ...
			  'MenuSelectedFcn','pplane10b(''restart'')',...
			  'separator','on');
       mequit = uimenu(fmenu,'label','Quit pplane10b',...
		       'MenuSelectedFcn','pplane10b(''quit'')','separator','off');
       ...
       menu(2) = uimenu(emenu,'label','Zoom in.',...
			'MenuSelectedFcn','pplane10b(''zoomin'')',...
			'pos',1);
       
       zsqmenu = uimenu(emenu,'label','Zoom in square.',...
			'MenuSelectedFcn','pplane10b(''zoominsq'')',...
			'pos',2);
       ...
       menu(3) = uimenu(megraph,'label',[txname,' vs. t'],...
			'MenuSelectedFcn','pplane10b(''plotxy'',1)');
       
       menu(5) = uimenu(megraph,'label',[tyname,' vs. t'],...
			'MenuSelectedFcn','pplane10b(''plotxy'',2)');
       
       meplot3 = uimenu(megraph,'label','Both',...
			'MenuSelectedFcn','pplane10b(''plotxy'',3)');
       
       meplot4 = uimenu(megraph,'label','3 D',...
			'MenuSelectedFcn','pplane10b(''plotxy'',4)');
       
       meplot5 = uimenu(megraph,'label','Composite',...
			'MenuSelectedFcn','pplane10b(''plotxy'',5)');
       ...
       menulevel = uimenu(solmenu,'label','Plot level curves.',...
			  'MenuSelectedFcn','pplane10b(''level'')',...
	 ...
	 metest = uimenu(solmenu,'label','Test case',...
			 'MenuSelectedFcn','pplane10b(''test case'')',...
       ...
       meset   = uimenu(menu(1),'label','Settings.',...
			'MenuSelectedFcn','pplane10b(''settings'')');
	...
	ode113ch = 'off';
	 ...
	 ode23tbch = 'off';
       ...
       meexportdata = uimenu(menu(1),'label','Export solution data.',...
			      'MenuSelectedFcn','pplane10b(''export'')',...
       ...
       menu(6) = uimenu(menu(1),'label','Make the Display Window inactive.',...
			'MenuSelectedFcn','pplane10b(''hotcold'')','separator','on');
	   ...
	   'MenuSelectedFcn','pplane10b(''showbar'')');
       
       set(0,'showhiddenhandles',hhsetup);
       
       set(gcf,'WindowButtonDownFcn','pplane10b(''down'')');
       set(ppdisp,'WindowButtonMotionFcn','pplane10b(''cdisp'')');
	...
	case 6
     ...
     ppkbd = findobj('name','pplane10b Keyboard input','vis','on');
     if ~isempty(ppkbd),pplane10b('kbd'),end
     pplevel = findobj('name','pplane10b Level sets');
...
elseif strcmp(action,'linear')
   ...
   % Get the information from pplane10b Equilibrium ...
   ...
   % Find pplane10b Linearization if it exists.	
   % If pplane10b Linearization exists, update it.  If it does not build it.
   
   pplin= findobj('name','pplane10b Linearization');
   if (~isempty(pplin))
      figure(pplin);
      dud = get(pplin,'user');
      dud.syst = system;
      ...
      pplin = figure('name','pplane10b Linearization',...
         ...
         'interrupt','on',...
         'visible','off',...
         'tag','pplane10b');
      pplane10b('figdefault',pplin);
      dud = get(pplin,'user');
      dud.dir = 0;
      dud.syst = system;
      dud.settings = settings;
      ...
      mequit = uimenu(fmenu,'label','Quit pplane10b',...
	  'MenuSelectedFcn','pplane10b(''quit'')','separator','on','pos',9);
	...
	chkd = 'on';
      ...
      set(pplin.Number,'WindowButtonDownFcn',['pplane10b(''down'',',num2str(pplin.Number),')']);
...
elseif strcmp(action,'dirfield')
   ...
   % Find pplane10b Display and get the user data.
   ...
   N = dud.syst.npts;
   if strcmp(get(disph,'name'),'pplane10b Linearization') 
      N = floor(3*N/4);
   end    
...
elseif strcmp(action,'hotcold')
   ...
   if (findstr(get(mehc,'label'),'inactive'))
      ...
      set(ppdisp,'WindowButtonDownFcn','pplane10b(''down'')');
...
elseif strcmp(action,'down')
  ...
  pplane10b('solution',initpt,disph);
...
elseif strcmp(action,'test case')
   ...
   for k = 1:10
      initpt = k*[-.2 .2];
      pplane10b('solution',initpt,ppdisp);
      initpt = k*[-.2 -.2];
      pplane10b('solution',initpt,ppdisp);
   end  
   pplane10b('solution',[1,-1],ppdisp);
   dud = get(gcf,'user');
   nstr = get(dud.notice,'string');
   nstr(1:4) = nstr(2:5);
   nstr{5,1} = 'Ready.';
...
elseif strcmp(action,'kcompute')
   ...
   % button on the pplane10b Keyboard figure.
   ...
   for kk = 1:6;
	 ...
	 xstr = pplane10b('paraeval',pn,pv,xstr);
	 ystr = pplane10b('paraeval',pn,pv,ystr);
       end
     end
   end
   xvalue = str2num(xstr);
...
elseif strcmp(action,'kbd')
   ...
   % sets up the pplane10b Keyboard figure which allows accurate input of 
   ...
   ppkbd = findobj('name','pplane10b Keyboard input');
   if  ~isempty(ppkbd)
     delete(ppkbd);
   end
         
   ppkbd = figure('name','pplane10b Keyboard input',...
		  'vis','off',...
		  'tag','pplane10b');
   
   pplane10b('figdefault',ppkbd);
   ...
   kbd.comp = uicontrol('style','push',...		
			'string','Compute','call','pplane10b(''kcompute'')');
   ...
   set(kbd.spec,'MenuSelectFcn',speccall);
   set(ppkbd,'resize','on');
...
%  z0 = ppginput(1);
  ...
  ppeqpt = findobj('name','pplane10b Equilibrium point data');
  
   if (isempty(ppeqpt))
     ppeqpt = figure('vis','off','resize','on',...
 		    'name','pplane10b Equilibrium point data',...
 		    'tag','pplane10b');
     pplane10b('figdefault',ppeqpt);
...
elseif strcmp(action,'zoomin')
   ...
   set(gcf,'WindowButtonDownFcn','pplane10b(''zoom'')',...
...
elseif strcmp(action,'zoom')
   ...
   set(axh,'user',aud);	
   set(disph,'user',dud);
   set(disph,'WindowButtonDownFcn','pplane10b(''down'')',...
      'WindowButtonUpFcn','');
   pplane10b('dirfield',disph);
   ppset = findobj('name','pplane10b Setup');
   if isempty(ppset)
      pplane10b('confused');
   else
...
elseif strcmp(action,'showbar')
  ...
  if strcmp(state,'figure')
	...
	'clickedcallback','pplane10b(''print'');');
    switch name
     case 'pplane10b Display'
      set(findobj(sbfig,'tooltipstr','Zoom Out'),...
	  'clickedcallback',['pplane10b(''zoomback'');' fixtb]);
      set(findobj(sbfig,'tooltipstr','Zoom In'),...
	  'clickedcallback',['pplane10b(''zoomin'');' fixtb]);
      delete(findobj(sbfig,'tooltipstr','Rotate 3D'));
      
     case 'pplane10b Linearization'
...
elseif strcmp(action,'settings')
   ...
   % up the pplane10b Settings window, which allows the user to interactively
   ...
   setfig = findobj('name','pplane10b Settings');
   if ~isempty(setfig);
     delete(setfig);
   end
   
   setfig = figure('name','pplane10b Settings',...
		   ...
		   'tag','pplane10b','vis','off');
   
   pplane10b('figdefault',setfig);
...
elseif strcmp(action,'setchange')
   ...
   % on the pplane10b Settings window.
...
elseif strcmp(action,'plotxy')  % Start a graph 
  ...
  x=get(objh,'xdata');
  y=get(objh,'ydata');
  hh = pplane10b('plotxyfig',type,gcf);
  set(ppdisp,'WindowButtonDownFcn',oldcall);
  aud = get(hh(2),'user');
  ud = get(hh(1),'user');
  ud.data = [t;x;y];
  set(hh(1),'user',ud);
  pplane10b('plxy',aud.rad,hh(1));
...
elseif strcmp(action,'plotxyfig')  % Build the graph figure.
  ...
  ppxy = figure('tag','pplane10b','visible','off');
  name = ['pplane10b t-plot # ',int2str(ppxy.Number)];
  ud.color = dud.color;
  ud.type = 0;
  set(ppxy,'name',name,'user',ud);
  pplane10b('figdefault',ppxy);
  ...
  set(findobj(fmenu,'label','Print Pre&view...'),'pos',6);
  set(findobj(fmenu,'label','&Print...'),'pos',7);
  qmenu = uimenu(fmenu,'label','Quit pplane10b',...
		 'MenuSelectedFcn','pplane10b(''quit'')',...
		 'separator','on');
...
elseif strcmp(action,'plxy')
   ...
   switch type
    case 1 
      ...
      ylabel(xstring)
      set(r3dh,'vis','off')
      set(fig,'WindowButtonDownFcn','pplane10b(''plxybdf'')');
    case 2
...
elseif strcmp(action,'crop')
   ...
   hh = pplane10b('plotxyfig',type,ppxy);
   ud = get(hh(1),'user');
   ud.data = [t;x;y];
   set(hh(1),'user',ud);
   aud = get(hh(2),'user');
...
elseif strcmp(action,'plxybdf')
   ...
   dispa = gca;
   dispf = gcf;
   set(dispf,'windowbuttonmotionfcn','pplane10b(''plxybmf'')',...
      'windowbuttonupfcn','pplane10b(''plxybuf'')',...
      'inter','on');
   aud = get(dispa,'user');
   delete(aud.h);
   aud.h = [];
...
elseif strcmp(action,'print')
   ...
   nstr = get(dud.notice,'string');
   nstr(1:4) = nstr(2:5);
   nstr{5} = 'Preparing to print the pplane10b Display Window.  Please be patient.';
   set(dud.notice,'string',nstr);
   
   nstr(1:4) = nstr(2:5);
   nstr{5} = 'Printing the pplane10b Display Window.';
   set(dud.notice,'string',nstr);
...
elseif strcmp(action,'figdefault')
  ...
  fig = input1;
  set(fig,'CloseRequestFcn','pplane10b(''closefcn'')');
  ppset = findobj('name','pplane10b Setup');
  sud = get(ppset,'user');
  ud = get(fig,'user');
  ud.ssize = sud.ssize;
...
elseif strcmp(action,'zoominsq')
   ...
   set(gcf,'WindowButtonDownFcn','pplane10b(''zoomsqd'')',...
      'WindowButtonUpFcn','1;','inter','on');
   set(gca,'inter','on');
   dud = get(gcf,'user');
   nstr = get(dud.notice,'string');
...
elseif strcmp(action,'zoomsqd')
   ...
   set(ppdispa,'user',aud);
   set(gcf,'windowbuttonmotionfcn','pplane10b(''zoomsqm'')',...
      'windowbuttonupfcn','pplane10b(''zoomsqu'')');
...
elseif strcmp(action,'zoomsqu')
   ...
   set(disph,'user',dud);
   set(disph,'WindowButtonDownFcn','pplane10b(''down'')',...
      'WindowButtonMotionFcn','pplane10b(''cdisp'')',...
      'WindowButtonUpFcn','');
   pplane10b('dirfield',disph);
   ppset = findobj('name','pplane10b Setup');
   if isempty(ppset)
...
elseif strcmp(action,'level')
  ...
  pplevel = figure('name','pplane10b Level sets',...
		 'vis','off',...
		 'tag','pplane10b');
  
  pplane10b('figdefault',pplevel);
  ...
  lev.rhs = uicontrol('style','edit','horiz','center',...
		      'string',' ','MenuSelectFcn','');

  lev.proc = uicontrol('style','push',...		
			  'string','Proceed',...
			  'call','pplane10b(''levcomp'')');
...
elseif strcmp(action,'levcomp')
  ...
  for kk = 1:4;
	...
	pv(find(abs(pv)==32))=[];
	lfcn = pplane10b('paraeval',pn,pv,lfcn);
      end
    end
  end
  l = length(lfcn);
...
elseif strcmp(action,'closefcn')
   ...
   fig = gcf;  
   name = get(fig,'name');
   if strcmp(name,'pplane10b Setup') | strcmp(name,'pplane10b Display')
      quest = ['Closing this window will cause all pplane10b ',...
            'windows to close, and pplane10b will stop.  ',...
            'Do you want to quit pplane10b?'];
      butt = questdlg(quest,'Quit pplane10b?','Quit','Cancel','Quit');
      if strcmp(butt,'Quit')
         pplane10b('quit');
      end
   elseif strcmp(name,'pplane10b Linearization')
      dud = get(fig,'user');
      fcn = dud.function;
...
elseif strcmp(action,'confused')
   ...
   tstring = 'pplane10b is totally confused';
   qstring = {['You will have to restart pplane10b from '...
      ...
      'What do you want to do?'};
   bstr1 = 'Quit and restart pplane10b.';
   bstr2 = 'Just quit pplane10b.';
   bstr3 = 'Do nothing.';
   answer = questdlg(qstring,tstring,bstr1,bstr2,bstr3,bstr1);
   if strcmp(answer,bstr1)
      delete(findobj('tag','pplane10b'));
      pplane10b;return
...
% PPOUT  is the output function for pplane10b.

%  Copywright (c)  John C. Polking, Rice University
%  Last Modified: January 21, 2001

output = 0;
ppdisp = findobj(get(0,'child'),'flat','name','pplane10b Display'); %gca;   % 
dud = get(ppdisp,'user');
ppdispa = dud.axes;
dfcn = dud.function;
ud = get(ppdispa,'user');
