function u_opt = FHCOP_Task4(xk, u_prev, Q, R, N, x_bar, u_bar, tau_s)
    % FHCOP_Task4
    % Solve the Finite Horizon Control Optimization Problem (FHOCP) with constraints on the maximum rate
    %
    % Arguments
    % - xk: The current state
    % - u_prev: The input at the previous time instant
    % - Q: The state weight
    % - R: The input weight
    ...
    % State constraints:  x_min <= x(k+i) <= x_max  
    
    % Input rate constaints:  delta_u_min <= u(k+i) - u(k+i-1) <= delta_u_max
